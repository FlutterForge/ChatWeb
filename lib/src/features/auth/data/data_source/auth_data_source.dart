...
Future<int> createUser(UserModel model) async {
  try {
    ...
    final Response response = await _dio.post(
      'http://http://192.168.1.31:8000/user',
      data: model.toJson(),
      options: Options(
        headers: {
    ...
    print("Response: ${response.data}, ${response.statusMessage}, ${response.statusCode}");

    if (response.statusCode == 200) {
      return response.data;  // Assuming your API returns a user ID or something similar
    } else {
    ...
    if (dioError.type == DioExceptionType.connectionError) {
      ...
      print('Dio error: ${dioError.response?.statusMessage}, ${dioError.response?.statusCode}');
    }
    rethrow;
  } catch (e) {
    print('Error: $e');
